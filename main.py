...
async def on_ready():
    ...
    print(f'Slot Bot by @DiscordTechBot | Akul xD\n\nLogged in as {bot.user.name}#{bot.user.discriminator} ({bot.user.id})\nPrefix: {prefix}\nOwner: {owner}\n')
    # Set the bot's status
    await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.listening, name="Akul xD Bot"))
@bot.command()
...
async def everyone(ctx):
    ...
    # Check if it's a new day and clear the command_usage dictionary if so
    if today not in command_usage:
        command_usage.clear()
        command_usage[today] = {}

    if (ctx.author.id, 'everyone') in command_usage[today]:
        if command_usage[today][(ctx.author.id, 'everyone')] >= 1:
            await ctx.send("`You have already used your @everyone for today.`")
            return
        else:
            command_usage[today][(ctx.author.id, 'everyone')] += 1
    else:
        command_usage[today][(ctx.author.id, 'everyone')] = 1
...
async def here(ctx):
    ...
    if (ctx.author.id, 'here') in command_usage[today]:
        if command_usage[today][(ctx.author.id, 'here')] >= 1:
            await ctx.send("`You have already used your @here for today.`")
            return
        else:
            command_usage[today][(ctx.author.id, 'here')] += 1
    else:
        command_usage[today][(ctx.author.id, 'here')] = 1
